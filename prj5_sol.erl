...
% functions.
dept_employees1(_, []) -> [];
dept_employees1(Dept, [#employee{dept = Dept} = E | Employees]) ->
  [E | dept_employees1(Dept, Employees)];
dept_employees1(Dept, [_ | Employees]) ->
  dept_employees1(Dept, Employees).
...
dept_employees2(Dept, Employees) ->
  lists:filter(fun
    (#employee{dept = Dept1}) when Dept1 == Dept -> true;
    (_) -> false
  end, Employees). 
...
dept_employees3(Dept, Employees) ->
  [E || E <- Employees, E#employee.dept == Dept].
...
delete_employee(Name, Employees) ->
  [E || E <-Employees, E#employee.name =/= Name].
...
% [E] appended.
upsert_employee(E, []) -> [E];
upsert_employee(#employee{name = N} = E, [#employee{name = N} | Employees]) ->
  [E | Employees];
upsert_employee(E, [E1 | Employees]) ->
  [E1 | upsert_employee(E, Employees)].
...
find_employees(Preds, Employees) ->
  PassesAll = fun (E) ->
    lists:all(fun (Pred) -> Pred(E) end, Preds)
  end,
  [E || E <- Employees, PassesAll(E)].
...
employees_req({delete, Name}, Employees) ->
  {ok, void, delete_employee(Name, Employees)};
employees_req({dump}, Employees) ->
  {ok, Employees, Employees};
employees_req({find, Preds}, Employees) ->
  {ok, find_employees(Preds, Employees), Employees};
employees_req({read, Name}, Employees) ->
  case find_employees([employee_has_name(Name)], Employees) of
    [Result] -> {ok, Result, Employees};
    _ -> {err, "NONE OR MORE THAN ONE WITH SAME NAME", Employees}
  end;
employees_req({upsert, E}, Employees) ->
  {ok, void, upsert_employee(E, Employees)};
employees_req(_, Employees) ->
  {err, "BAD REQUEST", Employees}.
...
employees_req_with_sort({sort}, Employees) ->
  Names = [Name || #employee{name = Name} <- Employees],
  % sort tuples by sorting fields from first to last
  % thus, sorting by name, since it is first in tuple
  {_, SortedByName} = lists:unzip(lists:sort(lists:zip(Names, Employees))),
  {ok, void, SortedByName};
employees_req_with_sort(Req, Employees) ->
  employees_req(Req, Employees).
...
start_employees_server(Employees, Fn) ->
    Server = spawn(fun () -> employees_server(Fn, Employees) end),
    true = register(emps, Server),
    Server.

employees_server(Fn, Employees) ->
  receive
    {_Pid, {stop}} -> terminated;
    {Pid, {new_fn, Fn1}} ->
      Pid ! {self(), {ok, void}},
      employees_server(Fn1, Employees);
    {Pid, Req} ->
      {Status, Result, Employees1} = Fn(Req, Employees),
      Pid ! {self(), {Status, Result}},
      employees_server(Fn, Employees1)
  end.
